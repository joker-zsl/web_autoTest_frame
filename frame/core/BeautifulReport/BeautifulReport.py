...
beautiful_report_dir_path = os.path.abspath(os.path.dirname(__file__))
...
class ReportTestResult(unittest.TestResult):
    ...
    def startTest(self, test) -> None:
        ...
        for handler in self.logger.handlers:
            if handler.stream == self.sys_stderr:
        ...
        if self.sys_stdout:
            for handler in self.logger.handlers:
                if handler.stream == sys.stderr:
                    handler.stream = self.sys_stderr
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        h, m = divmod(m, 60)
        resultData['totalTime'] = "{:02}时 {:02}分 {:02}秒".format(h, m, s)
        return resultData
...
if __name__ == '__main__':
    print(beautiful_report_dir_path)
